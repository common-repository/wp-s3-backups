...
class WPS3B {
	...
	function newBucketWarning() {
		echo "<div id='s3-warning' class='updated fade'><p><strong>".__('You need to select a valid S3 bucket.', 'wp-s3-backups')."</strong> ".__('If you tried to create a new bucket, it may have been an invalid name.', 'wp-s3-backups').' | <a href="options-general.php?page=' . plugin_basename(__FILE__) . '">'.__('Plugin Settings', 'wp-s3-backups').'</a></p></div>';
	...
	function add_settings_page() {
		...
		add_options_page(__('S3 Backup', 'wp-s3-backups'), __('S3 Backup', 'wp-s3-backups'), 8, __FILE__, array('WPS3B', 'settings_page'));	
	}
			...
			<div class="wrap">
				...
				<form method="post" action="options.php">
					...
					<p class="submit">
						...
						<input type="submit" id="backup-now" name="Submit" value="<?php _e('Save Changes and Backup Now', 'wp-s3-backups') ?>" />
					</p>
					<p>
						<?php 
							...
							foreach ( $crons as $cron ) {
								if ( isset($cron['s3-backup']) ) {
									...
									if ( $cron['args'][0] ) {
										printf(__("Your manual backup has started. If it hasn't yet, it will show up below once it's completed <a href='%s'>the next time you view this page.</a>", 'wp-s3-backups'), 'options-general.php?page=' . plugin_basename(__FILE__));
									}
								}
							}
						...
						if ( get_option('s3b-bucket') ) {
							...
							foreach ( $backups as $key => $backup ) {
								...
								if ( preg_match('|[0-9]{4}\.zip$|', $backup['name']) ) {
									$backup['label'] = sprintf(__('Manual WordPress Backup from %s', 'wp-s3-backups'), mysql2date(__('F j, Y h:i a'), date('Y-m-d H:i:s', $backup['time'])));
								} elseif ( preg_match('|[0-9]{4}\.uploads\.zip$|', $backup['name']) ) {
									$backup['label'] = sprintf(__('Manual Uploads Backup from %s', 'wp-s3-backups'), mysql2date(__('F j, Y h:i a'), date('Y-m-d H:i:s', $backup['time'])));
	...
	function backup($manual = false) {
		...
		if ( in_array('themes', $sections) ) $backups = array_merge($backups, WPS3B::rscandir(ABSPATH . 'wp-content/themes'));
		if ( in_array('plugins', $sections) ) $backups = array_merge($backups, WPS3B::rscandir(ABSPATH . 'wp-content/plugins'));
		...
		if ( !empty($backups) ) {
			...
			if ( $manual ) {
				$s3->putObject($upload, get_option('s3b-bucket'), next(explode('//', get_bloginfo('siteurl'))) . '/' . date('Y-m-d-Hi') . '.zip');
			} else {
		...
		if ( in_array('uploads', (array) $sections) ) {
			...
			$upload = $s3->inputFile($file);
			if ( $manual ) {
				$s3->putObject($upload, get_option('s3b-bucket'), next(explode('//', get_bloginfo('siteurl'))) . '/' . date('Y-m-d-Hi') . '.uploads.zip');
			} else {
	...
	function settings_link($links) {
		$settings_link = '<a href="options-general.php?page=' . plugin_basename(__FILE__) . '">' . __('Settings', 'wp-s3-backups') . '</a>'; 
...
add_action('s3-backup', array('WPS3B', 'backup'), 1);
add_action('init', array('WPS3B', 'init'));
add_filter("plugin_action_links_" . plugin_basename(__FILE__), array('WPS3B', 'settings_link') ); 
